...
# -*- coding: utf-8 -*-
"""
3倍株バックテスト: フェーズ1(52週新高値更新)の時点評価(v2・深い履歴版)
=====================================================================

find_3x_stocks_v2.py で取得した raw_cache_3x_v2/ (2022年まで遡ったキャッシュ)
を使って、真の52週新高値更新日をすべて洗い出す。ロジックは
jquants_screening.py の compute_new_highs() と同一。

ネットワーク不要・完全ローカルで動作します。

使い方:
  python3 phase1_newhighs_backtest_v2.py triple_events_true_YYYYMMDD.csv

出力: phase1_newhighs_v2_YYYYMMDD.csv
  (code, name, seq, new_high_date, high, prior_52w_high,
   true_low_date, true_high_date, true_multiple)
...
def compute_new_highs_until(rows, until_date: date):
    """d <= until_date の範囲で、実際に52週新高値を更新した日をすべて返す(古い順)"""
    normalized = []
    for r in rows:
...
def main():
    if not TRIPLE_EVENTS_FILE:
        print("使い方: python3 phase1_newhighs_backtest_v2.py triple_events_true_YYYYMMDD.csv")
        sys.exit(1)
    ...
    for t in triples:
        ...
        for seq, e in enumerate(events, 1):
            out_rows.append({
                ...
                "true_low_date": t["low_date"],
                "true_high_date": t["high_date"],
                "true_multiple": t["multiple"],
            })

    today = date.today()
    output_file = f"phase1_newhighs_v2_{today:%Y%m%d}.csv"
